...
/**
 * SensorThings API GET request decoder for observed properties.
 *
 * @author <a href="mailto:m.kiesow@52north.org">Martin Kiesow</a>
 */
public class StaGetObservedPropertiesRequestDecoder extends AbstractStaRequestDecoder<StaGetObservedPropertiesRequest> {
    ...
    public StaGetObservedPropertiesRequestDecoder() {
        super(StaGetObservedPropertiesRequest.class, StaConstants.SERVICE_NAME, StaConstants.VERSION_1_0,
                StaConstants.Operation.GET_OBSERVED_PROPERTIES.name());
    }

    @Override
    public StaGetObservedPropertiesRequest decodeJSON(JsonNode node, boolean validate) throws DecodingException {
        // node should be empty for all get requests

//        if (validate) {
//            JSONValidator.getInstance().validateAndThrow(node, getSchemaURI());
//        }
        StaGetObservedPropertiesRequest r = decodeRequest(node);
//        t.setExtensions(parseExtensions(node.path(JSONConstants.EXTENSIONS)));
        return r;
    }

    @Override
    public StaGetObservedPropertiesRequest decodeRequest(JsonNode node) throws DecodingException {
        // node should be empty for all get requests
        ...
        if (path.isEmpty()) {
            ...
            StaGetObservedPropertiesRequest request = new StaGetObservedPropertiesRequest(StaConstants.SERVICE_NAME,
                StaConstants.VERSION_1_0);

            // set ID
            if (resource.getId() != null && !resource.getId().equals("")) {
                try {
                    request.setId(Long.parseLong(resource.getId()));
