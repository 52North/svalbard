...
/**
 * Decoder for SensorThings API GET requests for datastreams.
 *
 * @author <a href="mailto:m.kiesow@52north.org">Martin Kiesow</a>
 */
public class StaGetDatastreamsRequestDecoder extends AbstractStaRequestDecoder<StaGetDatastreamsRequest> {
    ...
    public StaGetDatastreamsRequestDecoder() {
        super(StaGetDatastreamsRequest.class, StaConstants.SERVICE_NAME, StaConstants.VERSION_1_0,
                StaConstants.Operation.GET_DATASTREAMS.name());
    }

    @Override
    public StaGetDatastreamsRequest decodeJSON(JsonNode node, boolean validate) throws DecodingException {
        // node should be empty for all get requests

//        if (validate) {
//            JSONValidator.getInstance().validateAndThrow(node, getSchemaURI());
//        }
        StaGetDatastreamsRequest r = decodeRequest(node);
//        t.setExtensions(parseExtensions(node.path(JSONConstants.EXTENSIONS)));
        return r;
    }

    @Override
    public StaGetDatastreamsRequest decodeRequest(JsonNode node) throws DecodingException {
        // node should be empty for all get requests
        ...
        if (path.isEmpty()) {
            ...
            StaGetDatastreamsRequest request = new StaGetDatastreamsRequest(StaConstants.SERVICE_NAME,
