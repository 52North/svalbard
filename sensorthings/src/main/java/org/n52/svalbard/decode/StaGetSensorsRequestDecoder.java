...
/**
 * SensorThings API GET request for sensors.
 *
 * @author <a href="mailto:m.kiesow@52north.org">Martin Kiesow</a>
 */
public class StaGetSensorsRequestDecoder extends AbstractStaRequestDecoder<StaGetSensorsRequest> {
    ...
    public StaGetSensorsRequestDecoder() {
        super(StaGetSensorsRequest.class, StaConstants.SERVICE_NAME, StaConstants.VERSION_1_0, StaConstants.Operation.GET_SENSORS.name());
    }

    @Override
    public StaGetSensorsRequest decodeJSON(JsonNode node, boolean validate) throws DecodingException {
        // node should be empty for all get requests

//        if (validate) {
//            JSONValidator.getInstance().validateAndThrow(node, getSchemaURI());
//        }
        StaGetSensorsRequest r = decodeRequest(node);
//        t.setExtensions(parseExtensions(node.path(JSONConstants.EXTENSIONS)));
        return r;
    }

    @Override
    public StaGetSensorsRequest decodeRequest(JsonNode node) throws DecodingException {
        // node should be empty for all get requests
        ...
        if (path.isEmpty()) {
            ...
            StaGetSensorsRequest request = new StaGetSensorsRequest(StaConstants.SERVICE_NAME,
